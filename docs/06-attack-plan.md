...
| Audio | **AX, as a DSP micro-code** | The SDK's AX library runs recompiled and sends command lists to the DSP; the AX micro-code is emulated, as in Dolphin: a software mixer for its DSP-ADPCM / PCM8 / PCM16 voices, sample-rate conversion, volume, loops. Wwise, Bink and the Home Button all sit on it. Session 3 boots it silent |
...
| 5 | **Audio** ✅ | AX mixer. Target: music and voices, and a **rhythm game in sync** with Wwise's beat callbacks. Done in session 6: the AX micro-code in C++, everything heard, the rhythm game in time within the output's 20 ms or so (`13-audio.md`) |
| 6 | **PC finish** ✅ | window and fullscreen, higher internal resolution, widescreen (the engine has `vEnableWideScreen`), saves, key remapping, two players (the Home Button is already answered by the port's own box). Done in session 7: a SYSCONF at 16:9 gives full-height anamorphic frames, the window takes any shape, fullscreen is borderless, the internal resolution follows the window (× 3 at 1080 lines, 30 fps held), keys come from a file; saves were already on the host's NAND, and two players take turns on one Remote |
